...
class TestPhotoURLView(ModuleStoreTestCase, TestVerificationBase):
    ...
    def test_photo_url_view_returns_data(self):
        ...
        response = self.client.get("%s?receipt_id=%s" % (url, self.receipt_id))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data["EdX-ID"], self.receipt_id)

    def test_photo_url_view_returns_404_if_no_receipt_id(self):
        url = reverse('verification_photo_urls')
        response = self.client.get("%s?receipt_id=%s" % (url, 'aaa'))
